...
mkdir -p "${NODE_DIR}"
npx esbuild main.ts --bundle --platform=node --target=node20.5 --outfile=.sea/main.js
...
do
    ...
    cp "${NODE_DIR}/node-v${nodejs_version}-${os}/bin/node" ".sea/chromecast-mqtt-monitor-${os}"
    rm -rf "${NODE_DIR}/node-v${nodejs_version}-${os}"
    npx postject ".sea/chromecast-mqtt-monitor-${os}" NODE_SEA_BLOB .sea/prep.blob --sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2
done
